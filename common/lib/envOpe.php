...
class ENV_Info
{
    ...
    **/
    public function fileOutput($path, $file_name = "", $getProxy = false)
    {
        if (!$path) {
            die("ファイルパスが未指定です！ ※fileOutput実行時");
        }
        ...
        if ($getProxy) {
            if ($_ENV['HTTP_VIA']) {
                $pxy .= "HTTP_VIA:".$_ENV['HTTP_VIA']."\t";
            }
            if ($_ENV['HTTP_FORWARDED']) {
                $pxy .= "HTTP_FORWARDED:".$_ENV['HTTP_FORWARDED']."\t";
            }
            if ($_ENV['HTTP_X_FORWARDED_FOR']) {
                $pxy .= "HTTP_X_FORWARDED_FOR:".$_ENV['HTTP_X_FORWARDED_FOR']."\t";
            }
            if ($_ENV['HTTP_CACHE_INFO']) {
                $pxy .= "HTTP_CACHE_INFO:".$_ENV['HTTP_CACHE_INFO']."\t";
            }
            if ($_ENV['HTTP_XONNECTION']) {
                $pxy .= "HTTP_XONNECTION:".$_ENV['HTTP_XONNECTION']."\t";
            }
            if ($_ENV['HTTP_SP_HOST']) {
                $pxy .= "HTTP_SP_HOST:".$_ENV['HTTP_SP_HOST']."\t";
            }
            if ($_ENV['HTTP_FROM']) {
                $pxy .= "HTTP_FROM:".$_ENV['HTTP_FROM']."\t";
            }
            if ($_ENV['HTTP_X_LOCKING']) {
                $pxy .= "HTTP_X_LOCKING:".$_ENV['HTTP_X_LOCKING']."\t";
            }
            if ($_ENV['HTTP_PROXY_CONNECTION']) {
                $pxy .= "HTTP_PROXY_CONNECTION:".$_ENV['HTTP_PROXY_CONNECTION']."\t";
            }
        }
