...
class utilLib
{
    ...
    **/
    public static function strRep($str, $mode)
    {
        switch ($mode) {
            ...
            //改行コードを<br>に置き換えするモード ※置換結果は“<br>”となる（<br />にするならnl2brを使用する）
            case 3:
                if (!empty($str)) {
                    if (is_array($str)) {
                        for ($i=0;$i<count($str);$i++) {
            ...
            //stripslashesで\対策するモード（※php.iniのmagic_quotes_gpcがONの場合のみ）
            case 4:
            ...
            // 空白のコントロールコードと前後の半角及び全角の空白を強制除去 ※2004/2/6 見直し修正
            case 7:
                if (!empty($str)) {
                    if (is_array($str)) {
                        for ($i=0;$i<count($str);$i++) {
    ...
    public static function strCheck($str, $mode, $mes)
    {
        ...
        switch ($mode) {
            ...
            case 2:
                ...
                break;
            // 半角英数字以外の文字が使われていたらエラー
            case 3:
                if (preg_match("/[^_a-zA-Z0-9]/", $str)) {
                    $errmes .= "{$mes}<br>\n";
                }
                break;
            // 最後の文字に不正な文字が使われているかどうかのチェック。※URLやメール等。
            case 4:
                if (preg_match("/\.$|\/$|\@$|,$/", $str)) {
                    $errmes .= "{$mes}<br>\n";
                }
                break;
            // 不正かつ危険な文字が使われているかどうかのチェック。※URLやメール等。
            case 5:
                if (preg_match("/\||&|\/|\"|\\\/", $str)) {
                    $errmes .= "{$mes}<br>\n";
                }
                break;
            // メールアドレスチェック（形式がおかしかったらエラー）
            case 6:
                if (!preg_match("/^(.+)@(.+)\\.(.+)$/", $str)) {
                    $errmes .= "{$mes}<br>\n";
                }
    ...
    **/
    public static function httpHeadersPrint($charset="", $css_js_flg=false, $time_flg=false, $cache_flg=false, $robot_flg=false)
    {
        ...
        /* 引数１：文字コードの設定（省略時はutf-8。ここは必ず表示） */
        if (!$charset) {
            $charset = "utf-8P";
        }
